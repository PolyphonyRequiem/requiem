...
| 8 | Human gates in both terminal and web dashboard | 🟡 partial | Terminal ✅. **Web dashboard read-only viewer landed (ADR-0019, 2026-06-09):** `requiem.dashboard` — a stdlib-only (`http.server`, zero new deps) read-only projection of the event logs. Lists runs with status, renders a run's humanized event timeline, and surfaces the **pending human-gate queue** (`/api/gates`). Pure projection layer reuses the CLI's `_summarize_run` status semantics (`tests/test_dashboard.py`, 17). Launch: `python -m requiem.dashboard --log-dir .runs` (or the `requiem-dashboard` script). **Pending (phase 2, ADR-0019 §4):** resolving a gate *from* the browser (the write path) — the dashboard currently observes gates but an operator still services them via `requiem unblock` / the terminal gate handler. |
