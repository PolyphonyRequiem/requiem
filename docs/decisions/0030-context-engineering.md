...
- **2026-06-26 RUN #31 → 19/26 (73%) "valid CoderOutput" rate; bad_output root-caused to extractor + prompt; both fixed on `feat/run-31-followups`.** First run on top of the dual-clock + tool-isolation + cleanup + tok_in cap stack. **Headline result:** 19/26 leaves returned valid CoderOutput (vs run #30's 8/24 = 33%), 7 hit `bad_output`, 0 wedged/timed-out, 0 cascaded — the leaf-9 failure mode is gone. **However** post-run analysis surfaced two findings the headline number was hiding: **(1) Both runs #30 AND #31 were `dry_run=True`** because the launcher at `C:/tmp/run-requiem-commit.py` passed `--commit` (seeds ADO children) but NOT `--live` (lets the implementation workflow actually write files). `apply_changes` no-op'd on every leaf, `commit_changes`/`push_branch`/`create_pr` all reported `dry_run=True, sha=None, pushed=False, pr_number=None`. So what we have been measuring is "does the agent emit valid `CoderOutput` JSON", not "does shippable code land in a PR". **(2) All 7 bad_output failures shared one root cause:** the post-4e5ccf7 sealed-tool-surface causes sonnet-4.6 to emit valid CoderOutput JSON followed by Anthropic-native `<function_calls>` XML (the model wants to explore via tools but they are blocked). The trailing prose breaks `json.loads` with `Extra data: line 3 column 1 (char ~110)`. Receipt evidence: all 7 leaves had model=`claude-sonnet-4.6`, latency 7s-153s, 4572-4624 input_tokens, raw_output starts with `{"intent_summary":"Explore..."}` then `\n\nLet me explore...\n\n<function_calls>...`. **Two fixes shipped:** **(A) Extractor fall-through.** `_extract_json_block` step (1) "already-JSON whole text" used to always return `s` when it started with `{`, even when `json.loads(s)` would fail. Changed to TRY the parse first and fall through to step (3) brace-balanced extraction on `JSONDecodeError`. The balanced extractor was already correctly implemented; the bug was step (1) short-circuiting before step (3) got a chance. Empirically replayed the fix against all 7 run-#31 bad_output `raw_output` blobs: **7/7 now extract cleanly + validate as CoderOutput.** 4 new pin tests cover the trailing-prose case explicitly (object + array shapes, garbage trailing, plus a regression guard that whole-text-valid still passes through verbatim). **(B) Prompt no-tools directive.** `_build_prompt` now appends an explicit "you have NO tools available — do not emit `<function_calls>` / `<invoke>` / etc., any text after the JSON object will fail to parse" block at the TAIL of the prompt when a schema is set (same tail-placement rationale as the run-#25 schema-instruction fix at 551b414 — the model's most-recent constraint wins). Fix B is preventive (cuts off the tool-call attempt at the source); Fix A is the safety net for when the model attempts anyway. 2 new pin tests on the prompt (presence of the no-tools warning when schema is set; absence on no-schema agents). **Conductor prior-art consulted.** Conductor (`~/projects/conductor/src/conductor/providers/copilot.py:1044`) has an `_extract_json` plus an in-session **recovery loop** (`_build_parse_recovery_prompt`, up to 5 attempts) that quotes the parse error + truncated response back to the model. Recovery-loop is the canonical robust answer to "model emitted garbage"; we deferred shipping it as Fix C — Fix A + Fix B should empirically cover the cases we have observed, and a recovery loop is a heavier change. Re-evaluate after a `--live` run shows what failure modes survive. Suite: 1133/150/0 (4 new extractor pins + 2 new prompt pins on top of run-#30-followups' 12). **Open for the next session:** add `--live` to the launcher and rerun as `run #32` against `AB#62759077` to finally exercise the full `apply_changes` → `commit_changes` → `push_branch` → `create_pr` path. Defensive fixes from runs #30-#31 (cleanup, tok_in cap, extractor fall-through, no-tools prompt) all get live-fire tested for the first time.

- **2026-06-26 RUN #32 → FIRST `--live` LAUNCH. Fix B (no-tools prompt) PROVEN — bad_output 27% → 0%. New bottleneck: `run_tests` permanent-failure routing for 22/22 leaves.** First time `--live` was set on the launcher. The full pipeline executed for real: `trunk_bootstrap` created `feature/62759077` off main; `create_branch` produced real per-leaf branches; `commit_context_pack` landed real commits (verified — latest worktree HEAD is `e00f69c2 chore(context): requiem context pack for leaf 62880176 [plan_hash 3fe0342cf662]`); `invoke_coder` produced real CoderOutput JSON for all 22 leaves; `apply_changes` wrote 5-10 files per leaf to disk (real TypeScript modules + tests + YAML pipelines + PowerShell scripts; the cleanup verb scrubbed them before the workflow yielded). **HEADLINE FIX-B RESULT: 0/22 bad_output** — every leaf cleared the JSON-extraction gate cleanly, vs run-#31's 7/26 (27%) trailing-prose rate. Fix A (extractor fall-through) never had to fire as a safety net; Fix B (no-tools prompt directive) prevented the failure mode at source. The 1133/150/0 test suite was a real pin. **NEW BOTTLENECK — `run_tests` failure-mode routing.** Of 22 leaves: 21 routed to `end_needs_human` (`run_tests` returned `permanent_failure:tests.undetected` — "could not auto-detect a test command in C:\\...\\sku-fallback-worktree; pass test_command explicitly"); 1 routed to `end_failed` (leaf 62880161, `apply_changes` permanent-failed before `run_tests` could run — distinct shape, deferred). The CVAPI worktree has no `pyproject.toml` / `setup.py` / `setup.cfg` / `package.json` / `*.csproj` / `*.sln` at the root that `detect_test_command` (src/requiem/workflows/implementation.py:294) knows about. CVAPI is a C# service deployed via Ev2 with tests that run in Azure Pipelines, not a local `dotnet test`. **Run-#32 forecast scorecard:** "optimistic 25%: 18-22 land" was wrong-shaped (the 22 success-on-invoke_coder MATCHES the optimistic JSON-emission rate but the workflow can't ship because tests can't run). "Mid 40%: 10-14 land with new failure modes from apply/commit/push/PR" was directionally right — new failure mode surfaced — but it's `run_tests` not `commit_changes` / `push_branch` / `create_pr` (those never executed). **TAXONOMY (already in code, lines 936-976 of implementation.py): three `run_tests` error_kinds + a routing table.** **(1) `tests.undetected`** — no detectable test command in repo root. Currently routes to `cleanup_for_needs_human` (a future operator must manually pass `test_command`). 22/22 of run-#32's `run_tests` failures hit this. **(2) `tests.error`** — runner crashed (binary not found, etc.) — "we don't know what the truth is", per INV-NO-CORRUPT-FORWARD the human decides. Currently routes to `cleanup_for_needs_human`. 0/22 in run #32 (we never reached the runner because detection failed first). **(3) `tests.failed`** — tests RAN and returned non-zero exit. Currently routes to `invoke_coder_revision` (the model gets another shot with the test output). 0/22 in run #32. **Routing today (implementation.py:1271-1274):** `success → commit_changes`; `permanent_failure:tests.failed → invoke_coder_revision`; bare `permanent_failure → cleanup_for_needs_human` (catch-all for `tests.undetected` + `tests.error`). The catch-all is the current friction surface — both undetected and runner-crashed are funneled into the same "human escalate" terminal. **DELIBERATELY DEFERRED until taxonomy → per-mode policy is clearer:** no `--skip-tests` flag, no "soft pass when undetected" handler, no test_command override knob added to the launcher. Flat-flag policy would violate the standing operator rule (taxonomy → per-mode classify → group → THEN policy). Open question for the next session: what are the legitimate per-mode policies for each of `tests.undetected` / `tests.error` / `tests.failed` × leaf-type (atomic-code-only vs cross-cutting vs ops/docs)? E.g., an "Author Operational Runbook" task (docs-only) probably wants `tests.undetected` to be a SOFT PASS, but a "Capacity Signal Ingestion Pipeline" task should hard-fail because untested ingestion code is exactly the kind of thing we MUST not ship. Worth taxonomizing per CVAPI work-item type before any policy proposal. **State after run #32:** main @ `832e6ae`, requiem clean. CVAPI worktree clean on `main` @ `e00f69c2` (the cleanup-on-needs_human verb correctly scrubbed all 22 leaves' worktree state). `feature/62759077` trunk branch was created locally by `trunk_bootstrap` but never got per-leaf commits because `run_tests` failed before `commit_changes`. ADO AB#62759077 has 22 leaves + N intermediates seeded under the run-#32 planning (different IDs from run-#31's; cleanup before next launch).
