...
* `INV-EVENT-LOG-AUTHORITATIVE` is trivially honoured — nothing else
  persists.
* `INV-NO-CORRUPT-FORWARD`: a torn or garbled line raises
  `CorruptLogError` with the byte offset; the engine refuses to project
  past it.
"""
...
class CorruptLogError(RuntimeError):
    """Raised when an event-log line cannot be decoded as JSON."""

    def __init__(self, path: Path, line_no: int, byte_offset: int, detail: str) -> None:
        super().__init__(
...
@dataclass
class EventStore:
    """Append-only JSONL store. Assigns monotonically-increasing `event_id`."""
            ...
            for line_no, raw in enumerate(f, start=1):
                ...
                try:
                    ...
                    raise CorruptLogError(
                        self.path, line_no, f.tell() - len(raw), str(e)
                    ) from e
    ...
    with path.open("rb") as f:
        ...
        for line_no, raw in enumerate(f, start=1):
            ...
            try:
                ...
                raise CorruptLogError(path, line_no, start, str(e)) from e
