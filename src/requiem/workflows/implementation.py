...
def _bounded_path_bullets(paths: list[str]) -> list[str]:
    lines: list[str] = []
    used = 0
    for index, path in enumerate(paths):
        line = f"- `{path}`"
        if used + len(line) + 1 > MAX_PR_PATH_LIST_CHARS:
            lines.append(f"- ... {len(paths) - index} more path(s)")
            break
        lines.append(line)
        used += len(line) + 1
    return lines
...
def build_verb_registry(
    ...
    *,
    ...
    async def _create_pr(ctx):
        ...
        plan = ctx.completed["fetch_plan"]["value"]
        commit = ctx.completed.get("commit_changes", {}).get("value", {})
        item_title = str(plan["title"])
        title = f"AB#{plan['item_id']}: {item_title}"[:400]
        body_prefix = [
            f"Implements AB#{plan['item_id']} — {item_title[:300]}.",
            "",
            "## Plan",
            "",
        ]
        ...
        if commit.get("implementation_already_satisfied"):
            ...
            body_suffix.extend(
                _bounded_path_bullets(
                    [str(p) for p in commit.get("already_satisfied_paths", [])]
                )
            )
        body_suffix.extend([
            "",
        ...
        body_suffix.extend(
            _bounded_path_bullets(
                [str(p) for p in commit.get("files_changed", [])]
            )
        )
        body_suffix.extend([
            "",
        ...
        prefix = "\n".join(body_prefix)
        suffix = "\n".join(body_suffix)
        plan_text = str(plan.get("plan_text") or "(empty)")
        plan_budget = MAX_PR_BODY_CHARS - len(prefix) - len(suffix) - 2
        if len(plan_text) > plan_budget:
            excerpt_budget = max(0, plan_budget - len(PR_PLAN_TRUNCATION_MARKER))
            plan_text = (
                plan_text[:excerpt_budget].rstrip() + PR_PLAN_TRUNCATION_MARKER
            )
        body = f"{prefix}{plan_text}\n{suffix}"
