...
class OutcomeHandler(Protocol, Generic[T]):
    ...
    def on_weird_sixth(self, o: "WeirdSixth") -> T: ...  # NEW
...
class WeirdSixth(Outcome):
    ...
    def dispatch(self, handler: OutcomeHandler[T]) -> T:
        return handler.on_weird_sixth(self)

    def to_dict(self) -> dict[str, Any]:
        return {"kind": "weird_sixth", "reason": self.reason}
...
def main() -> int:
    # Explicit Protocol-typed binding makes the missing on_weird_sixth method fail mypy.
    engine: OutcomeHandler[Decision] = EngineHandler()
    ui: OutcomeHandler[str] = UiRenderHandler()
