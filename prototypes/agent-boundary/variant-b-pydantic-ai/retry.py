"""Retry wrapper for Variant B.

Mirrors variant A: same hard cap of 3 (north-star §4), same cancel
short-circuit (INV-CANCEL-SHORT-CIRCUITS-RETRY), same restamping of
``retry_key`` per attempt. Note: pydantic-ai *also* has its own internal
retry loop (``ModelRetry`` → up to ``retries`` re-prompts). The engine's
retry wrapper sits *outside* pydantic-ai's and only fires on Transient.
"""
