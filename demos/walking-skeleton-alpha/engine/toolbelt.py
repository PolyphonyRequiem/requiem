"""Liszt B+C hybrid — per-tool typed clients carried in a frozen `Toolbelt`.

Per-tool clients live in their own modules; verbs receive a `Toolbelt`
that bundles them. Verbs never know exit codes or argv — they pattern
match on the typed outcomes the clients return.

For the walking skeleton we wire two tiny clients: `git` (real, used to
read a snippet via `git show`) and `files` (a pure-Python file reader,
because the skeleton's snippet is sitting on disk in this repo). Both
have fake counterparts so the harness can drive the engine fully
in-process.
"""
...
class GitClient(Protocol):
    def show(self, repo: Path, ref: str, path: Path, *, timeout_s: float = 5.0) -> GitShowOutcome: ...
