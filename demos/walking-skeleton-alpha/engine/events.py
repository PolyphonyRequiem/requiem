"""Brahms B — envelope-loose events + typed emit helper.
...
class EventEmitter:
    ...
    Each `emit_*` accepts kwargs constrained at the call site; the
    envelope itself stays open so v2 readers can add kinds without
    breaking v1 (forward-compat for free).
    """
    ...
    def _emit(self, kind: str, *, node_id: str | None = None,
              team_id: str | None = None, agent_id: str | None = None,
              **payload: Any) -> int:
        envelope = {
            "run_id": self.run_id, "ts": _now().isoformat(),
            "kind": kind, "schema_version": SCHEMA_VERSION,
            "node_id": node_id, "team_id": team_id, "agent_id": agent_id,
            "payload": payload,
        }
    ...
    def emit_retry_attempted(self, node_id: str, attempt: int, next_attempt: int,
                             reason: str) -> None:
        self._emit("retry_attempted", node_id=node_id, attempt=attempt,
                   next_attempt=next_attempt, reason=reason)
    ...
    def emit_team_dispatched(self, node_id: str, team_id: str,
                             branches: list[str]) -> None:
        self._emit("team_dispatched", node_id=node_id, team_id=team_id,
                   branches=branches)

    def emit_team_branch_completed(self, node_id: str, team_id: str,
                                   agent_id: str, outcome: dict[str, Any]) -> None:
        self._emit("team_branch_completed", node_id=node_id, team_id=team_id,
                   agent_id=agent_id, outcome=outcome)

    def emit_gate_opened(self, node_id: str, prompt: str,
